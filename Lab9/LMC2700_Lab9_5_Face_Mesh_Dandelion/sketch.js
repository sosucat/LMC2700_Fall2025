// LMC 2700 Lab 9
// 5. Face Mesh Dandelion
// Blow into the digital world.
// Document: https://docs.ml5js.org/#/reference/facemesh
// Reference: https://youtu.be/FlBRSIz5AcQ
...
async function setup() {
  ...
  // Start camera (flipped) and hide the DOM element.
  video = createCapture(VIDEO, { flipped: true });
  video.size(640, 480);
  video.hide();

  // Helper to await p5 asset loading outside of preload.
  const loadFontAsync = (path) =>
    new Promise((resolve, reject) => loadFont(path, resolve, reject));

  // Load ml5 FaceMesh and fonts with await in setup.
  faceMesh = await ml5.faceMesh(options);
  fonts[0] = await loadFontAsync("fonts/BebasNeue-Regular.ttf");
  fonts[1] = await loadFontAsync("fonts/Kanit-Black.ttf");
  fonts[2] = await loadFontAsync("fonts/Roboto-Regular.ttf");

  // Start detection once model is ready.
  faceMesh.detectStart(video, gotFaces);

  // Build the rings of letters.
